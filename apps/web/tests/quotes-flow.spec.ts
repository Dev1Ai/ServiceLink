...
test.describe("Quotes accept/revoke flow", () => {
  ...
  test("customer accepts and revokes a quote", async ({ page, request }) => {
    ...
    const createJob = await request.post(`${api}/jobs`, {
      data: {
        title: `e2e-quotes-${Date.now()}`,
        description: "quotes flow test",
      },
      headers: {
    ...
    const createQuote = await request.post(
    ...
    await expect(card.locator("text=Status: accepted")).toBeVisible({
      timeout: 10000,
    });
    ...
    await expect(card.locator("text=Status: accepted")).toHaveCount(0, {
      timeout: 10000,
    });

...
test.describe("Scheduling workflow", () => {
  ...
  test("customer proposes schedule, provider confirms then rejects", async ({
    page,
    request,
  }) => {
    ...
    const jobRes = await request.post(`${api}/jobs`, {
      data: {
        title: `schedule-${Date.now()}`,
        description: "schedule flow test",
      },
      headers: {
    ...
    const quoteRes = await request.post(
      `${api}/jobs/${encodeURIComponent(job.id)}/quotes`,
      {
        data: { total: 33300 },
        headers: {
          "Content-Type": "application/json",
    ...
    const acceptRes = await request.post(
      `${api}/jobs/${encodeURIComponent(job.id)}/quotes/${encodeURIComponent(quote.id)}/accept`,
      {
    ...
    await page
      ...
      .first()
      .fill("Customer prefers mid-day window");
    await page
      .getByRole("button", { name: "Submit schedule proposal" })
      .click();

    await expect(
      page.locator("text=Schedule proposed successfully."),
    ).toBeVisible();
    ...
    await page
      .getByLabel("Confirmation notes (optional)")
      .fill("Provider confirms arrival");
    ...
    await page
      ...
      .fill("Need to reschedule quickly");
    await page
      .getByRole("button", { name: "Reject assignment and reopen job" })
      .click();

    await expect(
      page.locator("text=Assignment rejected — job reopened for quotes."),
    ).toBeVisible();
    await expect(page.locator("text=Status: provider_rejected")).toBeVisible({
      timeout: 10000,
    });

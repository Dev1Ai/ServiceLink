...
function parseMetrics(text: string): Sample[] {
  ...
  const re = /^(?<name>[a-zA-Z_:][a-zA-Z0-9_:]*)\s*(?<labels>\{[^}]*\})?\s+(?<value>[-+]?[0-9]*\.?[0-9]+)/;
...
function MetricsPageContent() {
  ...
  const [showNames, setShowNames] = useState<Record<InterestName, boolean>>(() => {
    ...
    return INTEREST.reduce((acc, n) => ((acc[n] = true), acc), {} as Record<InterestName, boolean>);
  });
  const [asRate, setAsRate] = useState<boolean>(() => {
    try { const saved = localStorage.getItem('metrics:rate'); if (saved != null) return saved === '1'; } catch {}
    return true;
  });
  const [windowSamples, setWindowSamples] = useState<number>(() => {
    try { const saved = localStorage.getItem('metrics:window'); if (saved) return Number(saved) || 60; } catch {}
    return 60;
  }); // ~10m at 10s interval

  const updateSeriesFromText = useCallback((text: string) => {
    const now = Date.now();
    const samples = parseMetrics(text);
    setSeries((prev) => {
      const next: Record<InterestName, Array<{ t: number; v: number }>> =
        prev || (INTEREST.reduce((acc, n) => ((acc[n] = [] as any), acc), {} as any) as Record<InterestName, Array<{ t: number; v: number }>>);
      (INTEREST as ReadonlyArray<InterestName>).forEach((name) => {
        const sum = samples.filter((s) => s.name === name).reduce((acc, s) => acc + (isFinite(s.value) ? s.value : 0), 0);
  ...
  // Persist UI settings
  useEffect(() => {
    try { localStorage.setItem('metrics:rate', asRate ? '1' : '0'); } catch {}
  }, [asRate]);
  useEffect(() => {
    try { localStorage.setItem('metrics:window', String(windowSamples)); } catch {}
  }, [windowSamples]);
  useEffect(() => {
    try { localStorage.setItem('metrics:show', JSON.stringify(showNames)); } catch {}
  }, [showNames]);
  ...
  useEffect(() => {
    try {
      ...
      if (show) {
        const set: Record<InterestName, boolean> = INTEREST.reduce((acc, n) => ((acc[n] = false), acc), {} as any);
  ...
  useEffect(() => {
    try {
      ...
      const selected = (INTEREST as ReadonlyArray<InterestName>).filter((n) => showNames[n]);
  ...
  return (
    <div className="container font-sans">
      ...
      <div className="controls">
        ...
        <label>window: 
          <select value={windowSamples} onChange={(e) => setWindowSamples(Number(e.target.value))}>
          ...
          <div key={name}>
            <label className="inline-flex gap-6 items-center">
              <input
                ...
                onChange={(e) => setShowNames((s) => ({ ...s, [name]: e.target.checked }))}
              />
              {name}
            </label>
            {showNames[name] && (
...
function MiniChart({ title, data }: { title: string; data: Array<{ t: number; v: number }> }) {
  ...
  useEffect(() => {
    ...
    const toY = (v: number) => y0 - (max === min ? 0.5 : (v - min) / (max - min)) * Hp;

...
export default function AssignmentsPage() {
  ...
  const load = useCallback(async () => {
    setStatus('Loading...');
    const res = await fetch(`${API}/providers/assignments`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const complete = async (id: string) => {
    setBusy(id);
    try {
      const res = await fetch(`${API}/providers/assignments/${id}/complete`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
      ...
      <div className="mt-16 grid gap-8">
        {items.map((a) => (
          <div key={a.id} className="card">
            <div><strong>Job:</strong> {a.job?.title || a.job?.key}</div>
            <div className="text-muted">{a.job?.description}</div>
            <div className="font-12 text-faint">Accepted: {new Date(a.acceptedAt).toLocaleString()}</div>
            {a.job?.id && (
              <div className="mt-8">
                ...
                {a.job?.key && (
                  <>
                    {' '}•{' '}
                    <Link href={`/realtime?room=${encodeURIComponent('job:' + a.job.key)}`}>Open chat</Link>
                  </>
                )}
              </div>
            )}

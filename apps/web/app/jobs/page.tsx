    ...
    <div className="container">
      ...
      <div className="flex gap-8 items-center">
        <input
          ...
          placeholder="paste CUSTOMER JWT here"
        />
      ...
      <div className="mt-8 grid gap-8">
        {filtered.map((j) => {
          ...
          const quotesHref = isStaticExport
            ? quotesStaticHref
            : `/jobs/${j.id}/quotes`;
          const quoteHref = isStaticExport
            ? quoteStaticHref
            : `/jobs/${j.id}/quote`;
          return (
            <div key={j.id} className={`card ${assigned ? "bg-assigned" : ""}`}>
              <div className="flex justify-between items-center">
                ...
                {assigned && (
                  <span
                    className={`badge ${status === "customer_verified" ? "badge-info" : "badge-success"}`}
                  >
              ...
              {assigned && (
                <div className="mt-6 font-13 text-green-strong">
                  Provider:{" "}
                  {prov?.name || prov?.email || j.assignment?.providerId}
                  {status && (
                    <span className="ml-8 text-subtle">• Status: {status}</span>
                  )}
                </div>
              )}
              <div className="font-12 text-faint mt-4">
                {new Date(j.createdAt).toLocaleString()}
              </div>
              <div className="mt-8 flex gap-8 flex-wrap">
                <Link href={quotesHref}>View quotes</Link>
                {!isStaticExport && (
                  <Link href={quotesStaticHref}>Quotes (static)</Link>
                )}
                <Link href={quoteHref}>Provider quote form</Link>
                {!isStaticExport && (
                  <Link href={quoteStaticHref}>Quote (static)</Link>
                )}
                {j.key && (
                  <Link
                    href={`/realtime?room=${encodeURIComponent("job:" + j.key)}`}
                  >
                    Open chat
                  </Link>
                )}

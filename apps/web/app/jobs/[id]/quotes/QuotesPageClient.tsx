...
const decodeJwtRole = (token: string): string => {
  if (typeof window === "undefined" || !token || token.split(".").length < 2)
    return "";
  try {
...
export default function QuotesPageClient({
  ...
  const [status, setStatus] = useState(
    isPlaceholder ? "Static export placeholder" : "",
  );
  const [job, setJob] = useState<JobResponse | null>(null);
  const [acceptCooldownUntil, setAcceptCooldownUntil] = useState<number | null>(
    null,
  );
  const [, forceTick] = useState(0);
  const [acceptInlineById, setAcceptInlineById] = useState<
  ...
  useEffect(() => {
    if (isPlaceholder) return;
    try {
      if (acceptCooldownUntil)
        localStorage.setItem(
          "quotes:acceptCooldownUntil",
          String(acceptCooldownUntil),
        );
      else localStorage.removeItem("quotes:acceptCooldownUntil");
    } catch {}
  }, [acceptCooldownUntil, isPlaceholder]);

  const acceptCdLeft = acceptCooldownUntil
    ? Math.max(0, Math.ceil((acceptCooldownUntil - Date.now()) / 1000))
    : 0;
  ...
  const accept = async (quoteId: string) => {
    if (isPlaceholder) {
      ...
      setAcceptInlineById((m) => ({
        ...m,
        [quoteId]:
          "Unavailable in static export; use /jobs/quotes?id=YOUR_JOB_ID.",
      }));
      return;
    }
    if (acceptDisabled) {
      ...
      setAcceptInlineById((m) => ({
        ...m,
        [quoteId]: `Please wait ~${acceptCdLeft}s before trying again.`,
      }));
      return;
    }
    setStatus("Accepting...");
    setAcceptInlineById((m) => ({ ...m, [quoteId]: "" }));
    const res = await fetch(`${API}/jobs/${id}/quotes/${quoteId}/accept`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    });
    await load();
    if (res.status === 429) {
      ...
      setAcceptInlineById((m) => ({
        ...m,
        [quoteId]: `Rate limited. Please wait ~${ttl}s before trying again.`,
      }));
    } else {
  ...
  const verifyComplete = async () => {
    if (isPlaceholder) {
      setStatus(
        "Static export placeholder — verifying completion is disabled.",
      );
      return;
    }
    setStatus("Verifying...");
  ...
  const revokeAcceptance = async () => {
    ...
    const res = await fetch(`${API}/jobs/${id}/quotes/revoke`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    });
    await load();
  ...
  const confirmSchedule = async () => {
    ...
    const res = await fetch(
      `${API}/assignments/${assignmentId}/schedule/confirm`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    <div className="container">
      ...
      {isPlaceholder && (
        <div className="alert alert-error">
          Static export placeholder — use /jobs/quotes?id=YOUR_JOB_ID when
          hosting the export.
        </div>
      )}
      {job && (
          ...
          {job.assignment && (
            <div className="mt-6 font-13 text-green-strong">
              ...
              {job.assignment.provider?.user?.name ||
                job.assignment.provider?.user?.email ||
                job.assignment.providerId}
              {job.assignment.status && (
                <span className="ml-8 text-subtle">
          ...
          {job.assignment && (
            <div className="mt-8 card border p-12 bg-muted">
              ...
              <div className="font-13 grid gap-4">
                <div>
                  <strong>Window:</strong> {scheduledWindow}
                </div>
                <div>
                  <strong>Version:</strong> {scheduleVersion}
                </div>
                <div>
                ...
                {job.assignment.scheduleNotes && (
                  <div>
                    <strong>Notes:</strong> {job.assignment.scheduleNotes}
                  </div>
                )}
                <div>
                  <strong>Reminder:</strong>{" "}
                  {job.assignment.reminderStatus || "N/A"}{" "}
                  {typeof job.assignment.reminderCount === "number" &&
                    `(count ${job.assignment.reminderCount})`}
                </div>
                {job.assignment.reminderLastSentAt && (
                  <div>
                    <strong>Reminder last sent:</strong>{" "}
                    {formatDate(job.assignment.reminderLastSentAt)}
                  </div>
                )}
                {job.assignment.payoutStatus && (
                  <div>
              ...
              {scheduleStatus && (
                <div className="alert mt-8">{scheduleStatus}</div>
              )}
              {role === "CUSTOMER" &&
                job.assignment.status !== "provider_rejected" && (
                  <div className="mt-12 grid gap-8">
                    ...
                    <button
                      onClick={() => proposeSchedule("CUSTOMER")}
              ...
              {role === "PROVIDER" &&
                job.assignment.status !== "provider_rejected" && (
                  <div className="mt-12 grid gap-8">
                    ...
                    <button
                      onClick={() => proposeSchedule("PROVIDER")}
                      disabled={isPlaceholder}
                    >
                      Submit schedule proposal
                    </button>
              ...
              {canConfirm && job.assignment.status !== "provider_rejected" && (
                <div className="mt-12 grid gap-6">
                  ...
                  <button onClick={confirmSchedule} disabled={isPlaceholder}>
                    Confirm schedule
                  </button>
                </div>
              )}
          ...
          {job.assignment && (
            <div className="mt-8 flex gap-8 items-center">
              <button
                ...
                title={
                  acceptDisabled
                    ? `Revoke cooldown: ${acceptCdLeft}s`
                    : "Revoke accepted quote"
                }
              >
                {acceptDisabled
                  ? `Revoke acceptance (cd ${acceptCdLeft}s)`
                  : "Revoke acceptance (customer)"}
              </button>
              {revokeStatus && (
                <span className="font-12 text-subtle">{revokeStatus}</span>
              )}
            </div>
          )}
        </div>
      ...
      <div className="flex gap-8 items-center">
        <input
          ...
          placeholder="paste JWT (customer or provider)"
          disabled={isPlaceholder}
        />
        <button
          onClick={() => {
      ...
      <div className="mt-16">
        <button onClick={verifyComplete} disabled={isPlaceholder}>
          Customer: Verify completion
        </button>
      </div>
      <div className="mt-16 grid gap-8">
          ...
          return (
            <div key={q.id} className={`card ${accepted ? "bg-accepted" : ""}`}>
              ...
              <div>
                ...
                {q.provider?.user?.name ||
                  q.provider?.user?.email ||
                  q.providerId}
              </div>
              <div>
              ...
              <div className="mt-8 flex gap-8">
                ...
                <button
                  ...
                  title={
                    acceptDisabled
                      ? isPlaceholder
                        ? "Unavailable in static export"
                        : `Accept cooldown: ${acceptCdLeft}s`
                      : "Accept quote"
                  }
                >
                  {acceptDisabled
        ...
        {quotes.length === 0 && (
          <div className="text-subtle">No quotes yet.</div>
        )}
      </div>
    </div>
  );

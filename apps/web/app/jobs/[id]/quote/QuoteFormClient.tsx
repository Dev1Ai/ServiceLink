...
export default function QuoteFormClient({
  ...
  const [inlineMsg, setInlineMsg] = useState<string | null>(
    isPlaceholder
      ? "Static export placeholder — use /jobs/quote?id=YOUR_JOB_ID when running from static hosting."
      : null,
  );
  const { push } = useToast();
  ...
  const submit = async () => {
    if (isPlaceholder) {
      setResult({
        ok: false,
        message:
          "Placeholder page in static export. Use /jobs/quote?id=YOUR_JOB_ID.",
      });
      setInlineMsg(
        "Static export placeholder — use /jobs/quote?id=YOUR_JOB_ID when hosting the export.",
      );
      return;
    }
    if (disabled) {
      setResult({
        ok: false,
        status: 429,
        message: `Cooldown active; wait ~${cdLeft}s`,
      });
      setInlineMsg(
        `Rate limited. Please wait ~${cdLeft}s before submitting again.`,
      );
    ...
    if (res.status === 429) {
      ...
      setResult({
        ok: false,
        status: 429,
        message: `Rate limited; wait ~${ttl}s`,
        data,
      });
      setInlineMsg(
        `Rate limited. Please wait ~${ttl}s before submitting again.`,
      );
    ...
    <div className="container">
      ...
      <div className="grid gap-8 max-w-420">
        ...
        <input
          ...
          placeholder="total (USD cents)"
          disabled={isPlaceholder}
        />
        {inlineMsg && <div className="alert alert-error">{inlineMsg}</div>}
        <button
          onClick={submit}
          disabled={disabled}
          title={

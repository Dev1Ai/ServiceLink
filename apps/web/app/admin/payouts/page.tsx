  ...
  const takeAction = async (
    ...
    try {
      const res = await fetch(
        `${API}/payments/payouts/${assignmentId}/${action}`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify(
            action === "deny" ? { reason: "manual review" } : {},
          ),
        },
      );
      if (res.ok) {
  ...
  return (
    <div className="container">
      ...
      <p className="text-muted font-13">
        Use an admin JWT to review assignments awaiting manual payouts.
      </p>
      ...
      <div className="mt-16 grid gap-12">
        {items.length === 0 && (
          <div className="text-muted">No assignments awaiting approval.</div>
        )}
            ...
            <div key={assignment.id} className="card">
              <div className="flex justify-between gap-12">
                <div>
                  ...
                  <div className="font-12 text-muted mt-4">
                ...
                <div className="text-right font-12 text-muted">
                  ...
                  <div>
                    Customer verified:{" "}
                    {assignment.customerVerifiedAt
                      ? new Date(assignment.customerVerifiedAt).toLocaleString()
                      : "—"}
                  </div>
                </div>
              </div>
              <div className="mt-12 flex gap-12">
                <button
                  onClick={() => takeAction(assignment.id, "approve")}
                ...
                <button
                  ...
                  onClick={() => takeAction(assignment.id, "deny")}
                  disabled={busy === assignment.id}
                >

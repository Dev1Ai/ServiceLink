...
function RealtimePageContent() {
  ...
  const [role, setRole] = useState<'CUSTOMER'|'PROVIDER'|'ADMIN'>('PROVIDER');
  ...
  const [messages, setMessages] = useState<Array<{id:string;room:string;userId:string;content:string;ts:string; user?: {id:string;email:string;name?:string}}>>([]);
  const [nextCursor, setNextCursor] = useState<string | undefined>(undefined);
  const socketRef = useRef<Socket | null>(null);
  const pingRef = useRef<any>(null);
  const [typingCooldownUntil, setTypingCooldownUntil] = useState<number | null>(null);
  ...
  useEffect(() => {
    try {
      if (typingCooldownUntil) localStorage.setItem('rt:typingCooldownUntil', String(typingCooldownUntil));
  ...
  useEffect(() => {
    try {
      if (chatCooldownUntil) localStorage.setItem('rt:chatCooldownUntil', String(chatCooldownUntil));
      else localStorage.removeItem('rt:chatCooldownUntil');
    } catch {}
  }, [chatCooldownUntil]);

  const typingCooldownLeft = typingCooldownUntil ? Math.max(0, Math.ceil((typingCooldownUntil - Date.now()) / 1000)) : 0;
  const chatCooldownLeft = chatCooldownUntil ? Math.max(0, Math.ceil((chatCooldownUntil - Date.now()) / 1000)) : 0;
  ...
  const connect = async () => {
    ...
    s.on('presence:update', (p: { online: string[] }) => setOnline(p.online || []));
    s.on('chat:message', (m: any) => setMessages((prev) => [...prev.slice(-49), m]));
    s.on('rate_limit', (p: { kind: 'typing' | 'chat'; ttl?: number; limit?: number }) => {
      const ttl = Number(p.ttl || 5);
      if (p.kind === 'typing') setTypingCooldownUntil(Date.now() + ttl * 1000);
  ...
  const login = async () => {
    ...
    try {
      const meRes = await fetch('http://localhost:3001/auth/me', { headers: { Authorization: `Bearer ${t}` } });
      if (meRes.ok) {
  ...
  const fetchHistory = async (cursorId?: string) => {
    ...
    if (!keyMatch) return;
    const url = new URL(`http://localhost:3001/jobs/${encodeURIComponent(keyMatch[1])}/messages`);
    url.searchParams.set('take', '50');
    if (cursorId) url.searchParams.set('cursorId', cursorId);
    const res = await fetch(url.toString());
    const data = await res.json();
    const items = (data.items || []) as Array<any>;
    setMessages(items.map((m) => ({ id: m.id, room, userId: m.userId, content: m.content, ts: m.createdAt, user: m.user })));
  ...
  const leaveRoom = () => { socketRef.current?.emit('room:leave', room); push(`Left ${room}`, 'info'); };
  ...
  return (
    <div className="container font-sans">
      ...
      <div className="grid-6 items-center">
        <input className="col-span-2" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="email" />
        <input className="col-span-2" value={nameField} onChange={(e) => setNameField(e.target.value)} placeholder="name" />
        <select className="col-span-1" value={role} onChange={(e) => setRole(e.target.value as any)}>
          <option value="CUSTOMER">CUSTOMER</option>
          <option value="PROVIDER">PROVIDER</option>
          <option value="ADMIN">ADMIN</option>
        </select>
        <input className="col-span-2" value={password} type="password" onChange={(e) => setPassword(e.target.value)} placeholder="password" />
        <button className="col-span-1" onClick={signup}>Signup</button>
        <button className="col-span-1" onClick={login}>Login</button>
        <button className="col-span-1" onClick={connect} disabled={!token}>Connect WS</button>
      </div>
      <div className="mt-16">
      ...
      <div className="flex gap-8 items-center">
        <input value={room} onChange={(e) => setRoom(e.target.value)} placeholder="room (e.g., job:123)" />
        <button onClick={joinRoom}>Join</button>
        <button onClick={leaveRoom}>Leave</button>
        <button onMouseDown={() => typing(true)} onMouseUp={() => typing(false)} disabled={typingDisabled} title={typingDisabled ? `Typing cooldown: ${typingCooldownLeft}s` : 'Emit typing while pressed'}>
          {typingDisabled ? `Hold to type (cd ${typingCooldownLeft}s)` : 'Hold to type'}
        </button>
      </div>
      <h3 className="mt-16">Online</h3>
      <pre className="pre">{JSON.stringify(online, null, 2)}</pre>
      <div>
        <button onClick={() => socketRef.current?.emit('presence:ping')}>Ping presence now</button>
      </div>
      ...
      <div className="flex gap-8 items-center">
        ...
        />
        <button onClick={sendChat} disabled={chatDisabled} title={chatDisabled ? `Chat cooldown: ${chatCooldownLeft}s` : 'Send message'}>
          {chatDisabled ? `Send (cd ${chatCooldownLeft}s)` : 'Send'}
        </button>
      </div>
      <div className="mt-8">
        <button disabled={!nextCursor} onClick={() => fetchHistory(nextCursor)}>Load older</button>
      </div>
      <div className="chat-box">
        {messages.map((m) => {
          const isMine = me?.id && m.userId === me.id;
          const who = isMine ? 'You' : (m.user?.name || m.user?.email || m.userId.slice(0,6));
          return (
            <div key={m.id} className={`msg-row ${isMine ? 'right' : 'left'}`}>
              <div className={`bubble ${isMine ? 'bubble-mine' : 'bubble-other'}`}>
                <div className="font-12 text-muted">{who} • {new Date(m.ts).toLocaleTimeString()}</div>

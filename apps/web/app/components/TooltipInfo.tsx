...
export function TooltipInfo({
  ...
  const placeClass = effPlacement === 'top' ? 'tt-top' : effPlacement === 'bottom' ? 'tt-bottom' : effPlacement === 'right' ? 'tt-right' : 'tt-left';
  ...
  return (
    ...
    >
      <span
        ...
        onClick={trigger === 'click' ? (e) => { e.stopPropagation(); setOpen((v) => !v); } : undefined}
        role="button"
        tabIndex={0}
        aria-expanded={open}
        aria-controls={wrapperRef.current ? `${(wrapperRef.current as any).__tid}` : undefined}
        aria-describedby={wrapperRef.current ? `${(wrapperRef.current as any).__tid}` : undefined}
        onKeyDown={(e) => {
      ...
      {open && (
        <div
          ...
          id={(wrapperRef.current && (wrapperRef.current as any).__tid) || undefined}
          ref={panelRef}
          role="tooltip"
        >
          <button

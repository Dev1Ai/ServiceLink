...
export default function ProvidersNearPage() {
  ...
  const [sort, setSort] = useState<"distance" | "price" | "online" | "rank">(
    "distance",
  );
  const [rank, setRank] = useState<"balanced" | "cheap" | "near" | "online">(
    "balanced",
  );
  ...
  useEffect(() => {
    ...
    const loadMine = async () => {
      ...
      try {
        ...
        if (pl && (pl.lat != null || pl.lng != null))
          setMyLoc({ lat: pl.lat, lng: pl.lng });
      } catch {}
    };
    loadMine();
    ...
    try {
      ...
      if (raw) {
        ...
        if (f.sort === "distance" || f.sort === "price" || f.sort === "online")
          setSort(f.sort);
    ...
    <div className="container">
      ...
      <div className="grid-6 items-center max-w-1000">
        ...
        <input
          ...
          placeholder="service contains (optional)"
        />
        <select
          className="col-span-1"
          value={category}
        ...
        <button className="col-span-1" onClick={search}>
          Search
        </button>
        <button
          className="col-span-1"
        ...
        <input
          ...
          placeholder="Selected Job ID (for quick quote)"
        />
        <div className="col-span-6 flex gap-8 items-center">
          ...
          <button
            ...
            title="Provider-only; requires provider JWT"
          >
            Set my location (provider)
          </button>
      ...
      <div className="mt-16 grid gap-8">
        {myLoc && (
          <div className="text-subtle">
            My stored location: {myLoc.lat?.toFixed(5)}, {myLoc.lng?.toFixed(5)}
          </div>
        )}
        {items.map((p: any) => (
...
function LeafletMap({
  ...
  return (
    ...
    >
      ...
      {isFinite(lat as number) &&
        isFinite(lng as number) &&
        isFinite(radiusKm as number) &&
        radiusKm! > 0 && (
          <Circle
            center={[lat as number, lng as number]}
            radius={(radiusKm as number) * 1000}
      ...
      {providers
        ...
        .map((p) => (
          <CircleMarker
            key={p.id}
            center={[
              ((p as any).lat as number) || 0,
              ((p as any).lng as number) || 0,
            ]}
            pathOptions={{
              color: (p as any) && (p as any).online ? "#16a34a" : "#9ca3af",
            }}
            radius={6}
          >
            ...
            <Popup>
              <div className="min-w-220">
                <div className="font-600 flex items-center gap-8">
                  {p.user?.name || p.user?.email || p.id}
                  <span
                    className={`font-11 ${(p as any).online ? "text-green" : "text-gray-500"}`}
                  >
                ...
                <div className="mt-8">
                  <a
                    href={`/realtime?room=${encodeURIComponent("provider:" + p.id)}`}
                  >
                    Open chat
                  </a>
                  {selectedJobId && (
                    <>
                      ...
                      •{" "}
                      <a
                        href={`/jobs/${encodeURIComponent(selectedJobId)}/quote`}
                      >
                        Quote this job
                      </a>
                    </>

        ...
        {nodes.map((n) => (
          <li key={n.id} className="my-6">
            <div className="flex gap-8 items-center">
              ...
              <a href={`/providers/search?category=${encodeURIComponent(n.slug)}`}>Search</a>
              <a href={`/providers/near?category=${encodeURIComponent(n.slug)}`}>Near</a>
            </div>
            {n.children && n.children.length ? render(n.children, level + 1) : null}
          </li>
        ))}
      </ul>
    );
  ...
  return (
    <div className="container">
      ...
      {!tree.length && status === 'OK' && (
        <div className="text-muted">No categories found. Seed the database or add categories.</div>
      )}
    </div>
  );
}

...
type Prov = { id: string; user?: { name?: string; email?: string }; online?: boolean; services?: Array<{ id: string; name: string; price?: number; description?: string }> };
...
export default function ProvidersSearchPage() {
  ...
  const [services, setServices] = useState<Array<{ name: string; count: number }>>([]);
  ...
  useEffect(() => {
    try {
      const f = { lat, lng, radiusKm, q, onlineOnly, minPrice, maxPrice, sort, order, page, take, service, category };
      localStorage.setItem('providers:search:filters', JSON.stringify(f));
    } catch {}
  }, [lat, lng, radiusKm, q, onlineOnly, minPrice, maxPrice, sort, order, page, take, service, category]);

  return (
    <div className="container">
      <h2>Providers Search</h2>
      <div className="map-frame">
        <LeafletMap providers={items as any} myLat={parseFloat(lat)} myLng={parseFloat(lng)} radiusKm={parseFloat(radiusKm)} />
      </div>
      <div className="grid-6 items-center max-w-1000">
        <input className="col-span-2" value={lat} onChange={(e) => setLat(e.target.value)} placeholder="lat (optional)" />
        <input className="col-span-2" value={lng} onChange={(e) => setLng(e.target.value)} placeholder="lng (optional)" />
        <div className="col-span-1 flex items-center gap-6">
          <input className="flex-1" value={radiusKm} onChange={(e) => setRadiusKm(e.target.value)} placeholder="radius km" />
          <TooltipInfo trigger="click" placement="top" theme="light" text={'When lat/lng are set, radius filters results\nwithin the specified distance of your location.'} />
        </div>
        <div className="col-span-2 flex gap-8">
          <button onClick={geo}>Use current location</button>
          <button onClick={geo10}>Use my location + 10km</button>
        </div>
        <input className="col-span-2" value={q} onChange={(e) => setQ(e.target.value)} placeholder="service contains" />
        ...
        <select className="col-span-1" value={service} onChange={(e) => setService(e.target.value)}>
        ...
        <label className="col-span-1 flex gap-6 items-center">
          ...
          <TooltipInfo trigger="click" placement="top" theme="light" text={'Show only providers currently marked online.'} />
        </label>
        <div className="col-span-6 font-12 text-muted">
          <TooltipInfo trigger="click" placement="top" theme="light" text={'Service filters by a specific service name;\nCategory filters by a broader group of services.'} />
          <span className="ml-8">Tip: combine category and service for precise results.</span>
        </div>
        ...
        <button className="col-span-1" onClick={() => { setPage(1); search(); }}>Search</button>
        <button
          className="col-span-1"
          onClick={() => {
            setLat(''); setLng(''); setRadiusKm(''); setQ(''); setOnlineOnly(false);
            setMinPrice(''); setMaxPrice(''); setSort('price'); setOrder('asc'); setService(''); setCategory(''); setPage(1);
            setTimeout(search, 0);
          }}
        >
          Clear filters
        ...
        <div className="col-span-3 flex gap-8 items-center">
          ...
          <select value={take} onChange={(e) => { setTake(parseInt(e.target.value)); setPage(1); setTimeout(search, 0); }}>
...
function LeafletMap({ providers, myLat, myLng, radiusKm }: { providers: any[]; myLat?: number; myLng?: number; radiusKm?: number }) {
  ...
  const firstWithCoords = providers.find((p) => Number.isFinite(p?.lat) && Number.isFinite(p?.lng));
  const center: any = Number.isFinite(myLat) && Number.isFinite(myLng)
    ? [myLat as number, myLng as number]
    : (firstWithCoords ? [firstWithCoords.lat, firstWithCoords.lng] : defaults);

  return (
    <MapContainer center={center} zoom={12} className="h-full w-full" scrollWheelZoom>
      <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution="&copy; OpenStreetMap contributors" />
      {Number.isFinite(myLat) && Number.isFinite(myLng) && (
        <CircleMarker center={[myLat as number, myLng as number]} pathOptions={{ color: '#2563eb' }} radius={8}>
          <Tooltip direction="top">You</Tooltip>
        </CircleMarker>
      )}
      {Number.isFinite(myLat) && Number.isFinite(myLng) && (
        <Circle center={[myLat as number, myLng as number]} radius={1000 * (Number.isFinite(radiusKm as number) && (radiusKm as number) > 0 ? (radiusKm as number) : 5)} pathOptions={{ color: '#60a5fa', fillOpacity: 0.05 }} />
      )}
      {providers
        .filter((p) => Number.isFinite(p?.lat) && Number.isFinite(p?.lng))
        .map((p) => (
          <CircleMarker key={p.id} center={[p.lat, p.lng]} pathOptions={{ color: p.online ? '#16a34a' : '#9ca3af' }} radius={6}>
            ...
            <Popup>
              <div className="min-w-220">
                <div className="font-600 flex items-center gap-8">
                  ...
                  <span className={`font-11 ${p.online ? 'text-green' : 'text-gray-500'}`}>{p.online ? 'Online' : 'Offline'}</span>
                </div>
                {p.minServicePrice != null && Number.isFinite(p.minServicePrice) && (
                  <div className="font-12 text-subtle">From ${((p.minServicePrice || 0) / 100).toFixed(2)}</div>
                )}
                <div className="font-12 text-subtle">{(p.services || []).map((s: any) => s.name).join(', ') || 'No services listed'}</div>
              </div>
            </Popup>
          </CircleMarker>
        ))}

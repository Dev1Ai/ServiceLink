...
describe('Quotes E2E (HTTP)', () => {
  ...
  it('400 on invalid body', async () => {
    prisma.provider.findUnique.mockResolvedValue({ id: 'prov1', userId: 'user-prov-1' });
    prisma.job.findUnique.mockResolvedValue({ id: 'job1' });
    await request(app.getHttpServer()).post('/jobs/job1/quotes').send({ total: 0 }).expect(400);
  });

  it('404 when job missing', async () => {
    prisma.provider.findUnique.mockResolvedValue({ id: 'prov1', userId: 'user-prov-1' });
    prisma.job.findUnique.mockResolvedValue(null);
    await request(app.getHttpServer()).post('/jobs/job-missing/quotes').send({ total: 100 }).expect(404);
  });
  ...
  it('201 on success', async () => {
    ...
    prisma.quote.create.mockResolvedValue({ id: 'q1', jobId: 'job1', providerId: 'prov1', total: 150, status: 'pending' });
    await request(app.getHttpServer())
      .post('/jobs/job1/quotes')
      .send({ total: 150 })
      .expect(201)
      .expect(({ body }) => expect(body).toMatchObject({ id: 'q1', total: 150 }));
  });
});

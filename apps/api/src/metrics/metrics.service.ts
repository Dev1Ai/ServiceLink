...
export class MetricsService {
  ...
  incWsConnect(role: string, redis: boolean) {
    this.wsConnectCounter.labels(role?.toUpperCase?.() || 'UNKNOWN', String(!!redis)).inc();
  }
...
export class MetricsHttpHelper {
  ...
  record(method: string, route: string, statusCode: number, seconds: number) {
    ...
    this.metrics.recordHttpDuration(labels.method, labels.route, Number(labels.status_code), seconds);
  }
}

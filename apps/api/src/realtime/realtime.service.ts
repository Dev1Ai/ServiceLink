...
export class RealtimeService {
  ...
  async addOnline(userId: string) {
    if (!userId) return;
    if (this.redis) {
      try { await this.redis.sadd(this.setKey, userId); return; } catch {}
    }
  ...
  async removeOnline(userId: string) {
    if (!userId) return;
    if (this.redis) {
      try { await this.redis.srem(this.setKey, userId); return; } catch {}
    }
  ...
  async listOnline(): Promise<string[]> {
    if (this.redis) {
      try { return (await this.redis.smembers(this.setKey)) || []; } catch {}
    }
    const now = Date.now();
  ...
  async count(): Promise<number> {
    if (this.redis) {
      try { return (await this.redis.scard(this.setKey)) ?? 0; } catch {}
    }
  ...
  async touchAlive(userId: string, ttlSeconds = 60) {
    if (!userId) return;
    if (this.redis) {
      try { await this.redis.setex(`presence:alive:${userId}`, ttlSeconds, '1'); return; } catch {}
    }
  ...
  async reconcile(_ttlSeconds = 60): Promise<void> {
    ...
    if (this.redis) {
      try {
        const members: string[] = (await this.redis.smembers(this.setKey)) || [];

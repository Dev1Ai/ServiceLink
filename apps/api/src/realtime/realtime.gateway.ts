  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async afterInit() {
    ...
    if (redisUrl) {
      try {
        ...
        this.logger.warn(
          "Redis adapter not available; falling back to in-memory presence",
        );
      }
    }
  ...
  private async authenticate(client: Socket): Promise<JwtPayload | null> {
    try {
      ...
      const auth = client.handshake.headers["authorization"] as
        | string
        | undefined;
  ...
  async handleConnection(client: Socket) {
    ...
    (client.data as { userId?: string; role?: string }).role = (
      payload.role || ""
    )
      .toString()
      .toUpperCase();
    ...
    if (this.posthog)
      this.posthog.capture({
        ...
        event: "ws_connect",
        properties: { role: payload.role, redis: this.redisAdapterReady },
      });
    ...
    client.on("typing", (data: { room: string; isTyping: boolean }) => {
      ...
      if (this.posthog)
        this.posthog.capture({
          distinctId: userId,
          event: "ws_typing",
          properties: { room: data.room, isTyping: !!data.isTyping },
        });
    ...
    client.on("chat:send", async (data: { room: string; content: string }) => {
      ...
      if (match) {
        ...
        const saved = await this.prisma.chatMessage.create({
          data: { jobId: job.id, userId, content: data.content },
        });
        id = saved.id;
  ...
  ): { allowed: boolean; ttl?: number; limit?: number } {
    try {
      const cfg = this.config;
      const baseTtl = Number(
        cfg.get<string>(
          kind === "typing" ? "WS_TYPING_RATE_TTL" : "WS_CHAT_RATE_TTL",
        ) ?? (kind === "typing" ? 10 : 60),
      );
      const baseLimit = Number(
        cfg.get<string>(
          kind === "typing" ? "WS_TYPING_RATE_LIMIT" : "WS_CHAT_RATE_LIMIT",
        ) ?? (kind === "typing" ? 10 : 15),
      );
      const cTtl = Number(
        cfg.get<string>(
          kind === "typing"
            ? "WS_TYPING_RATE_TTL_CUSTOMER"
            : "WS_CHAT_RATE_TTL_CUSTOMER",
        ) ?? baseTtl,
      );
      const cLimit = Number(
        cfg.get<string>(
          kind === "typing"
            ? "WS_TYPING_RATE_LIMIT_CUSTOMER"
            : "WS_CHAT_RATE_LIMIT_CUSTOMER",
        ) ?? baseLimit,
      );
      const pTtl = Number(
        cfg.get<string>(
          kind === "typing"
            ? "WS_TYPING_RATE_TTL_PROVIDER"
            : "WS_CHAT_RATE_TTL_PROVIDER",
        ) ?? baseTtl,
      );
      const pLimit = Number(
        cfg.get<string>(
          kind === "typing"
            ? "WS_TYPING_RATE_LIMIT_PROVIDER"
            : "WS_CHAT_RATE_LIMIT_PROVIDER",
        ) ?? baseLimit,
      );
      let ttl = baseTtl;
      let limit = baseLimit;
      ...
      type RateStore = {
        getRecordSync?: (key: string) => number[];
        getRecord?: (key: string) => Promise<number[]>;
        addRecord?: (key: string, ttl: number) => Promise<void> | void;
      };
  ...
  async handleDisconnect(client: Socket) {
    const userId = (client.data as { userId?: string }).userId as
      | string
      | undefined;
    if (userId) {
      ...
      const pingTimer: NodeJS.Timeout | undefined = (
        client.data as { pingTimer?: NodeJS.Timeout }
      ).pingTimer;
      if (pingTimer) clearInterval(pingTimer);
      this.broadcastPresence();
      if (this.posthog)
        this.posthog.capture({ distinctId: userId, event: "ws_disconnect" });
    }
  }

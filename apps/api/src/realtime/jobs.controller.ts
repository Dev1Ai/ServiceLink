...
export class JobsController {
  ...
  @Get(":key/messages")
  @ApiOperation({
    summary: "List recent chat messages for a job room (job:<key>)",
  })
  @ApiQuery({
    name: "take",
    required: false,
    description: "Number of messages to return (1-200), default 50",
  })
  @ApiQuery({
    name: "cursorId",
    required: false,
    description: "Message id cursor; returns older messages before this id",
  })
  ...
  async listMessages(
    ...
    const take = Math.max(
      1,
      Math.min(200, Number.parseInt(takeQ ?? "50", 10) || 50),
    );

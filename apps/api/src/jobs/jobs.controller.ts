 ...
 * - Providers: view available jobs, list assignments, create quotes for jobs
...
export class JobsController {
  ...
  @Roles("PROVIDER")
  @ApiBearerAuth("bearer")
  @ApiOperation({ summary: "List available jobs for provider to quote on" })
  @ApiOkResponse({ type: [JobDto] })
  async listAvailable(@Req() req: AuthedRequest) {
    const userId = req.user.sub;
    ...
    // Return jobs without assignments that the provider hasn't quoted on yet
    return this.prisma.job.findMany({
      where: {
        assignment: null,
        quotes: {
  ...
  async listProviderAssignments(@Req() req: AuthedRequest) {
    const userId = req.user.sub;
    const provider = await this.prisma.provider.findUnique({
      where: { userId },
    });
  ...
  @ApiBearerAuth("bearer")
  @ApiOperation({
    summary: "Create job (customer)",
    description:
      "Rate limited via jobs limiter; configure with JOBS_RATE_* env vars",
  })
  @ApiOkResponse({ type: JobDto })
  ...
  async createFromAudio(
    ...
    return this.jobs.createJobFromAudio(file, req.user.sub);
  }
  ...
  @ApiBearerAuth("bearer")
  @ApiOperation({
    summary: "Customer proposes a schedule window for the assigned provider",
  })
  @ApiOkResponse({ type: AssignmentDto })
  @ApiBadRequestResponse({ type: ErrorDto })
  ...
  @Roles("PROVIDER")
  @ApiBearerAuth("bearer")
  @ApiOperation({ summary: "Draft a quote for a job using AI (provider)" })
  @ApiOkResponse({ type: CreateQuoteDto })
  async draftQuote(
    @Param("id") id: string,
    @Req() req: AuthedRequest,
  ): Promise<CreateQuoteDto> {
    return this.jobs.draftQuote(id, req.user.sub);
  }
  ...
  @ApiBearerAuth("bearer")
  @ApiOperation({
    summary: "Accept a quote (customer)",
    description:
      "Rate limited via quotes limiter; configure with QUOTES_RATE_* env vars",
  })
  @ApiOkResponse({ type: QuoteDto })
  @ApiBadRequestResponse({
    description:
      "Reasons include: assignment already exists, a quote already accepted, target quote not pending, or invalid quote/job pair",
    type: ErrorDto,
  })
  @ApiTooManyRequestsResponse({
    description: "Too many accept attempts",
    type: ErrorDto,
    headers: {
      "Retry-After": {
        description: "Seconds to wait before retrying",
  ...
  @ApiBearerAuth("bearer")
  @ApiOperation({
    summary: "Revoke an accepted quote (customer)",
    description:
      "Rate limited via quotes limiter; configure with QUOTES_RATE_* env vars",
  })
  @ApiOkResponse({
  ...
  @ApiBadRequestResponse({
    description: "No active acceptance to revoke or not allowed",
    type: ErrorDto,
  })
  @ApiTooManyRequestsResponse({
    description: "Too many revoke attempts",
    type: ErrorDto,
    headers: {
      "Retry-After": {
        description: "Seconds to wait before retrying",
  ...
  async customerComplete(@Param("id") id: string, @Req() req: AuthedRequest) {
    ...
    this.logger.log(
      `Payment capture mode for job ${id}: ${paymentResult.mode}`,
    );
    return updated;
  }
}

...
export class AssignmentsService {
  ...
  async proposeScheduleAsCustomer(jobId: string, customerId: string, dto: ProposeScheduleDto) {
    ...
    if (job.customerId !== customerId) throw new ForbiddenException('Not allowed');
    if (!job.assignment) throw new BadRequestException('Job has no active assignment');
    ...
    const result = await this.updateAssignmentSchedule(job.assignment.id, job.assignment.scheduleVersion, version, {
  ...
  async proposeScheduleAsProvider(assignmentId: string, providerUserId: string, dto: ProposeScheduleDto) {
    const { start, end, version, notes } = dto;
    this.ensureWindow(start, end);
    ...
    if (assignment.provider.userId !== providerUserId) throw new ForbiddenException('Not allowed');
    if (assignment.status === ASSIGNMENT_STATUS.PROVIDER_REJECTED) {
      throw new BadRequestException('Assignment already rejected');
    }
    const result = await this.updateAssignmentSchedule(assignment.id, assignment.scheduleVersion, version, {
      scheduledStart: start,
      scheduledEnd: end,
  ...
  async confirmSchedule(assignmentId: string, actor: { userId: string; role: 'CUSTOMER' | 'PROVIDER' }, dto: ConfirmScheduleDto) {
    const assignment = await this.prisma.assignment.findUnique({
      where: { id: assignmentId },
      select: {
        id: true,
    ...
    if (actor.role === 'CUSTOMER' && assignment.job.customerId !== actor.userId) {
      throw new ForbiddenException('Not allowed');
    }
    if (actor.role === 'PROVIDER' && assignment.provider.userId !== actor.userId) {
      throw new ForbiddenException('Not allowed');
    }
    const result = await this.updateAssignmentSchedule(assignment.id, assignment.scheduleVersion, dto.version, {
  ...
  async completeAssignmentAsProvider(assignmentId: string, providerUserId: string) {
    const assignment = await this.prisma.assignment.findUnique({
      where: { id: assignmentId },
      select: {
        id: true,
    ...
    if (assignment.status === ASSIGNMENT_STATUS.CUSTOMER_VERIFIED) {
      return this.prisma.assignment.findUnique({ where: { id: assignment.id } });
    }
    const now = new Date();
  ...
  async rejectAssignment(assignmentId: string, providerUserId: string, dto: RejectAssignmentDto) {
    const assignment = await this.prisma.assignment.findUnique({
      where: { id: assignmentId },
      select: {
        id: true,
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.quote.updateMany({ where: { jobId: assignment.jobId, status: 'accepted' }, data: { status: 'withdrawn_by_provider' } });
      await tx.quote.updateMany({ where: { jobId: assignment.jobId, status: 'declined' }, data: { status: 'pending' } });
      return updateResult;
    });
  ...
  private async updateAssignmentSchedule(
    ...
    if (update.count === 0) {
      throw new ConflictException('Schedule was updated by another action. Refresh and retry.');
    }
    const refreshed = await this.prisma.assignment.findUnique({ where: { id: assignmentId } });
    if (!refreshed) {

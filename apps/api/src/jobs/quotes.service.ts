...
export class QuotesService {
  ...
  async createQuote(jobId: string, userId: string, dto: CreateQuoteDto) {
    ...
    const existing = await this.prisma.quote.findFirst({ where: { jobId, providerId: provider.id } });
    if (existing) throw new BadRequestException('You already submitted a quote for this job');
    try {
      const created = await this.prisma.quote.create({ data: { jobId, providerId: provider.id, total: dto.total } });
      await this.notifications.notifyQuoteCreated(jobId, created.id, provider.id);
      return created;
    } catch (e: unknown) {
      const err = e as { code?: string };
      if (err?.code === 'P2002') {
        throw new BadRequestException('You already submitted a quote for this job');
      }
      throw e;
    }
  }
  ...
  async listQuotes(jobId: string, userId: string) {
    const job = await this.prisma.job.findUnique({ where: { id: jobId }, select: { id: true, customerId: true } });
    if (!job) return [];
  ...
  async acceptQuote(jobId: string, quoteId: string, userId: string) {
    ...
    const acceptedCount = await this.prisma.quote.count({ where: { jobId: job.id, status: 'accepted' } });
    if (acceptedCount > 0) {
      throw new BadRequestException('A quote has already been accepted for this job');
    }
    const quote = await this.prisma.quote.findUnique({ where: { id: quoteId }, select: { id: true, jobId: true, status: true, providerId: true } });
    ...
    await this.prisma.$transaction(async (tx) => {
      const updated = await tx.quote.updateMany({ where: { id: quoteId, jobId: job.id, status: 'pending' }, data: { status: 'accepted' } });
      if (updated.count !== 1) {
    ...
    await this.notifications.notifyQuoteAccepted(jobId, quoteId, quote.providerId);
  ...
  async revokeAcceptance(jobId: string, userId: string) {
    ...
    const job = await this.prisma.job.findUnique({ where: { id: jobId }, select: { id: true, customerId: true } });
    if (!job || job.customerId !== userId) throw new ForbiddenException('Not allowed');
    // Find accepted quote and assignment
    const accepted = await this.prisma.quote.findFirst({ where: { jobId, status: 'accepted' } });
    const assignment = await this.prisma.assignment.findUnique({ where: { jobId } });
    if (!accepted || !assignment) throw new BadRequestException('No active acceptance to revoke');
    // Revoke within a transaction
    await this.prisma.$transaction(async (tx) => {
      await tx.quote.update({ where: { id: accepted.id }, data: { status: 'pending' } });
      await tx.assignment.delete({ where: { jobId } });
      await tx.quote.updateMany({ where: { jobId, status: 'declined' }, data: { status: 'pending' } });
    });

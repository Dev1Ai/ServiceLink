...
describe("JobsController - quotes", () => {
  ...
  const metricsMock = {
    incPaymentInitiate: jest.fn(),
  } as unknown as MetricsService;
  const paymentsMockImpl = {
  ...
  it("throws Forbidden when provider profile missing", async () => {
    ...
    await expect(
      controller.createQuote("job1", reqWithUser("user1"), {
        total: 100,
      } as any),
  ...
  it("throws NotFound when job does not exist", async () => {
    ...
    await expect(
      controller.createQuote("job-missing", reqWithUser("user1"), {
        total: 120,
      } as any),
  ...
  it("creates quote on happy path", async () => {
    ...
    const result = await controller.createQuote("job1", reqWithUser("user1"), {
      total: 300,
    } as any);
  ...
  it("calls payments service when customer verifies completion", async () => {
    ...
    const result = await controller.customerComplete(
      "job1",
      reqWithUser("cust1"),
    );
    expect(result.status).toBe("customer_verified");
    expect(metricsMock.incPaymentInitiate).toHaveBeenCalledWith("job_complete");
    expect(paymentsMockImpl.handleCustomerVerification).toHaveBeenCalledWith(
      "job1",
    );
  });
});

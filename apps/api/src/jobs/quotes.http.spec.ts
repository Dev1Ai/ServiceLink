...
describe('Jobs/Quotes HTTP (E2E-lite)', () => {
  ...
  const paymentsImpl = { handleCustomerVerification: jest.fn().mockResolvedValue({ mode: 'manual' }) };
  ...
  it('POST /jobs/:id/quotes/:quoteId/accept returns 400 when assignment exists', async () => {
    prisma.job.findUnique.mockResolvedValue({ id: 'job1', customerId: 'cust1', assignment: { id: 'a1' } });

    const req = { user: { sub: 'cust1' } } as AuthedRequest;
    await expect(controller.acceptQuote('job1', 'q1', req)).rejects.toThrow('Assignment already exists');
  ...
  it('POST /jobs/:id/quotes/:quoteId/accept returns 400 when quote is not pending', async () => {
    prisma.job.findUnique.mockResolvedValue({ id: 'job1', customerId: 'cust1', assignment: null });
    prisma.quote.count.mockResolvedValue(0);
    prisma.quote.findUnique.mockResolvedValue({ id: 'q1', jobId: 'job1', status: 'accepted', providerId: 'prov1' });

    const req = { user: { sub: 'cust1' } } as AuthedRequest;
    await expect(controller.acceptQuote('job1', 'q1', req)).rejects.toThrow('Quote is not pending');
  ...
  it('POST /jobs/:id/quotes/revoke returns 400 when no acceptance exists', async () => {
    ...
    const req = { user: { sub: 'cust1' } } as AuthedRequest;
    await expect(controller.revokeAccepted('job1', req)).rejects.toThrow('No active acceptance');
  ...
  it('POST /jobs/:id/complete verifies completion and updates assignment', async () => {
    ...
    const updatedAssignment = { id: 'assign1', status: 'customer_verified', customerVerifiedAt: new Date().toISOString() };
    ...
    expect(paymentsImpl.handleCustomerVerification).toHaveBeenCalledWith('job1');
  });
});

...
export class ProvidersRoleLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const baseLimit = Number(
      this.config.get<string>("PROVIDERS_RATE_LIMIT") ?? 5,
    );
    const provTtl = Number(
      this.config.get<string>("PROVIDERS_RATE_TTL_PROVIDER") ?? baseTtl,
    );
    const provLimit = Number(
      this.config.get<string>("PROVIDERS_RATE_LIMIT_PROVIDER") ?? baseLimit,
    );
    ...
    if (totalHits > limit) {
      ...
      throw new HttpException(
        "Rate limit exceeded for provider actions",

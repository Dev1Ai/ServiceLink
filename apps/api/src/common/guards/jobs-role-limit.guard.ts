...
export class JobsRoleLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const custTtl = Number(
      this.config.get<string>("JOBS_RATE_TTL_CUSTOMER") ?? baseTtl,
    );
    const custLimit = Number(
      this.config.get<string>("JOBS_RATE_LIMIT_CUSTOMER") ?? baseLimit,
    );
    const provTtl = Number(
      this.config.get<string>("JOBS_RATE_TTL_PROVIDER") ?? baseTtl,
    );
    const provLimit = Number(
      this.config.get<string>("JOBS_RATE_LIMIT_PROVIDER") ?? baseLimit,
    );

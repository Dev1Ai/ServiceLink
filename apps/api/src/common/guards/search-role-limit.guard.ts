...
export class SearchRoleLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const baseLimit = Number(
      this.config.get<string>("SEARCH_RATE_LIMIT") ?? 30,
    );
    const custTtl = Number(
      this.config.get<string>("SEARCH_RATE_TTL_CUSTOMER") ?? baseTtl,
    );
    const custLimit = Number(
      this.config.get<string>("SEARCH_RATE_LIMIT_CUSTOMER") ?? baseLimit,
    );
    const provTtl = Number(
      this.config.get<string>("SEARCH_RATE_TTL_PROVIDER") ?? baseTtl,
    );
    const provLimit = Number(
      this.config.get<string>("SEARCH_RATE_LIMIT_PROVIDER") ?? baseLimit,
    );
    ...
    if (totalHits > limit) {
      ...
      throw new HttpException(
        "Rate limit exceeded for search",

...
export class ProvidersService {
  ...
  async createOnboardingLink(userId: string) {
    ...
    const looksPlaceholder =
      !stripeSecret ||
      /\*/.test(String(stripeSecret)) ||
      /sk_live_or_test/.test(String(stripeSecret));
    if (looksPlaceholder) {
      this.logger.warn(
        "STRIPE_SECRET_KEY not set. Returning mock onboarding URL.",
      );
    ...
    const returnUrl =
      this.config.get<string>("STRIPE_RETURN_URL") ||
      "http://localhost:3000/provider/onboarding/completed";
    const refreshUrl =
      this.config.get<string>("STRIPE_REFRESH_URL") ||
      "http://localhost:3000/provider/onboarding/refresh";
    try {
      ...
      this.logger.warn(
        `Stripe error creating account link; returning mock. ${String(err)}`,
      );
      return { url: "https://connect.stripe.com/setup/mock" };
    }
  }

...
import {
  OnboardingLinkDto,
  ProvidersNearResponseDto,
  ProvidersSearchResponseDto,
...
export class ProvidersController {
  ...
  @ApiOperation({
    summary: "Create Stripe Connect onboarding link for provider",
  })
  ...
  @Get("search")
  @ApiOperation({
    summary: "Search providers by service name (simple)",
    description:
      "Rate limited via search limiter; configure with SEARCH_RATE_* env vars",
  })
  @ApiOkResponse({
    description: "Paginated providers",
    type: ProvidersSearchResponseDto,
  })
  @ApiQuery({
    name: "q",
    required: false,
  ...
  @ApiQuery({
    name: "service",
    required: false,
    description: "Exact service name filter (case-insensitive)",
  })
  @ApiQuery({
    name: "category",
    required: false,
    description: "Category slug filter (case-insensitive)",
  })
  @ApiQuery({
    name: "onlineOnly",
    required: false,
    description: "Filter to online providers",
    schema: { type: "boolean" },
  })
  @ApiQuery({
    name: "minPrice",
  ...
  @ApiQuery({
    name: "sort",
    required: false,
    description: "Sort by price | online",
    schema: { type: "string", default: "price" },
  })
  @ApiQuery({
    name: "order",
    required: false,
  ...
  @ApiQuery({
    name: "lat",
    required: false,
    description: "Latitude for optional radius filter",
    schema: { type: "number" },
  })
  @ApiQuery({
    name: "lng",
    required: false,
    description: "Longitude for optional radius filter",
    schema: { type: "number" },
  })
  @ApiQuery({
    name: "radiusKm",
    required: false,
    description: "Radius (km) for optional location filter",
    schema: { type: "number" },
  })
  ...
  async search(@Query() dto: SearchProvidersQueryDto) {
    ...
    if (isFinite(minPrice))
      filters.push({
        services: { some: { price: { gte: Number(minPrice) } } },
      });
    if (isFinite(maxPrice))
      filters.push({
        services: { some: { price: { lte: Number(maxPrice) } } },
      });
    if (serviceName)
      filters.push({
        services: {
          some: { name: { equals: serviceName, mode: "insensitive" } },
        },
      });
    if (categorySlug)
      filters.push({
    ...
    const where: Prisma.ProviderWhereInput = filters.length
      ? { AND: filters }
      : {};
    ...
    const useRadius =
      isFinite(lat) && isFinite(lng) && isFinite(radiusKm) && radiusKm > 0;
    ...
    if (useRadius) {
      ...
      const R = 6371;
      type RowWithDist = Prisma.ProviderGetPayload<{
        select: typeof radiusSelect;
      }> & {
        distanceKm: number;
      ...
      const withDist: RowWithDist[] = rows
        .map((p) => {
          ...
          const a =
            Math.sin(dLat / 2) ** 2 +
            Math.cos(toRad(lat)) *
              Math.cos(toRad(p.lat || 0)) *
              Math.sin(dLng / 2) ** 2;
          const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
          const dist = R * c;
          const minServicePrice = p.services.reduce(
            (acc: number, s: { price: number | null }) =>
              typeof s.price === "number"
                ? Math.min(acc, s.price || Infinity)
                : acc,
            Infinity,
          );
          return {
            ...p,
            distanceKm: dist,
            minServicePrice: isFinite(minServicePrice) ? minServicePrice : null,
          };
        })
        .filter(
          (p) =>
            p.distanceKm <= (radiusKm || 25) &&
            (!p.serviceRadiusKm || p.distanceKm <= p.serviceRadiusKm),
        );
      ...
      withDist.sort((a, b) => {
        if (sort === "online") {
          ...
          return (
            (order === "asc" ? av - bv : bv - av) || a.distanceKm - b.distanceKm
          );
        }
  ...
  @ApiOperation({
    summary:
      "Find providers near lat/lng within radiusKm (and optional service name q)",
    description:
      "Rate limited via search limiter; configure with SEARCH_RATE_* env vars",
  })
  @ApiOkResponse({
    description: "Paginated providers with distanceKm",
    type: ProvidersNearResponseDto,
  })
  ...
  @ApiQuery({
    name: "sort",
    required: false,
    description: "Sort by distance | price | online",
  ...
  async near(@Query() req: NearProvidersQueryDto) {
    ...
    if (!isFinite(lat) || !isFinite(lng))
      return { items: [], total: 0, page, take, hasNext: false };
      ...
      .sort((a, b) => {
        if (sort === "rank") {
          const weights = (() => {
            switch (rank) {
              case "cheap":
                return { wd: 0.3, wp: 0.6, wo: 0.1 };
              case "near":
                return { wd: 0.7, wp: 0.2, wo: 0.1 };
              case "online":
                return { wd: 0.4, wp: 0.2, wo: 0.4 };
              default:
                return { wd: 0.5, wp: 0.4, wo: 0.1 };
            }
          })();
    ...
    return {
      ...
      page,
      take,
      hasNext: start + paged.length < withDist.length,
    };
  }
  ...
  @ApiOkResponse({
    description: "List of quotes with job info",
  ...
  @ApiOkResponse({
    description: "List of assignments with job info",
    schema: { type: "array", items: { type: "object" } },
  })
  ...
  @ApiOkResponse({
    description: "Array of { name, count }",
    schema: {
      type: "array",
      items: {
        type: "object",
  ...
  async listCategories() {
    ...
    rows.forEach((r) =>
      map.set(r.id, { id: r.id, name: r.name, slug: r.slug, children: [] }),
    );
    const roots: CatNode[] = [];
    rows.forEach((r) => {

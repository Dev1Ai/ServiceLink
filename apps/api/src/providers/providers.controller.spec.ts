...
describe("ProvidersController (unit)", () => {
    ...
    prisma.provider.findMany.mockResolvedValue([
      {
        ...
        user: { name: "N", email: "n@example.com" },
      },
      {
        id: "far",
        userId: "u2",
        ...
        user: { name: "F", email: "f@example.com" },
      },
    ]);
    const res = await controller.near({
      lat: 0,
      lng: 0,
      radiusKm: 25,
      page: 1,
  ...
  it("categories: returns a tree", async () => {
    ...
    expect(tree[0].children.map((c: any) => c.name).sort()).toEqual([
      "Lawn",
      "Plumbing",
    ]);
  });

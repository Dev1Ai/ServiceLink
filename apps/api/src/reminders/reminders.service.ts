...
import {
  REMINDER_QUEUE_NAME,
  REMINDER_QUEUE_PROVIDER,
  ReminderQueueResources,
...
export class RemindersService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    @Inject(REMINDER_QUEUE_PROVIDER)
    private readonly resources: ReminderQueueResources | null,
  ) {
    this.leadMinutes = Number(
      this.config.get<string>("REMINDER_LEAD_MINUTES") ?? 30,
    );
    this.lookaheadMinutes = Math.max(
      this.leadMinutes,
      Number(this.config.get<string>("REMINDER_LOOKAHEAD_MINUTES") ?? 120),
    );
    this.overdueMinutes = Number(
      this.config.get<string>("REMINDER_OVERDUE_MINUTES") ?? 30,
    );
    this.pollIntervalMs = Number(
      this.config.get<string>("REMINDER_POLL_INTERVAL_MS") ?? 300000,
    );
    this.concurrency = Number(
      this.config.get<string>("REMINDER_WORKER_CONCURRENCY") ?? 2,
    );
    const enabledFlag =
      (
        this.config.get<string>("REMINDER_WORKER_ENABLED") ?? "true"
      ).toLowerCase() !== "false";
    ...
    if (!this.enabled) {
      this.logger.warn(
        "Assignment reminder worker disabled (set REMINDER_WORKER_ENABLED=true and ensure REDIS_URL is configured).",
      );
    }
  }
  ...
  async onModuleInit() {
    ...
    this.worker.on("failed", (job, err) => {
      this.logger.error(
        `Reminder job ${job?.id as string} failed`,
        err as Error,
      );
    ...
    this.pollTimer = setInterval(() => {
      this.scanAndEnqueue().catch((err) =>
        this.logger.error("Reminder scan failed", err as Error),
      );
      this.markOverdue().catch((err) =>
        this.logger.error("Mark overdue failed", err as Error),
      );
    }, this.pollIntervalMs);
  ...
  async scanAndEnqueue() {
    ...
    const lookahead = new Date(
      now.getTime() + this.lookaheadMinutes * 60 * 1000,
    );
    ...
    for (const assignment of assignments) {
      ...
      try {
        ...
        queued += 1;
        this.logger.log(
          `Queued reminder for assignment ${assignment.id} (delay=${Math.round(delay / 1000)}s).`,
        );
      } catch (err) {
        this.logger.error(
  ...
  private async handleReminder(data: ReminderJobData) {
    ...
    if (!assignment) {
      this.logger.warn(
        `Reminder job skipped — assignment ${data.assignmentId} not found`,
      );
      return;
    }
    ...
    if (assignment.scheduleVersion !== data.scheduleVersion) {
      this.logger.log(
        `Reminder job skipped — assignment ${assignment.id} schedule version changed`,
      );
      return;
    }
    ...
    if (assignment.status !== "scheduled") {
      this.logger.log(
        `Reminder job skipped — assignment ${assignment.id} status is ${assignment.status}`,
      );
      return;
    }
    ...
    if (assignment.reminderStatus === AssignmentReminderStatus.SENT) {
      this.logger.log(
        `Reminder job skipped — assignment ${assignment.id} already reminded`,
      );
      return;
    }

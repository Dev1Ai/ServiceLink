...
export const ReminderQueueProvider: Provider = {
  ...
  useFactory: async (
  ...
  ): Promise<ReminderQueueResources | null> => {
    const logger = new Logger("ReminderQueueProvider");
    const redisUrl = config.get<string>("REDIS_URL");
    const enabledFlag =
      (
        config.get<string>("REMINDER_WORKER_ENABLED") ?? "true"
      ).toLowerCase() !== "false";
    if (!enabledFlag) {
      logger.warn(
        "REMINDER_WORKER_ENABLED set to false – reminder queue disabled.",
      );
      return null;
    }
    if (!redisUrl) {
    ...
    try {
      ...
      logger.error(
        "Failed to connect to Redis for reminders queue",
        err as Error,
      );

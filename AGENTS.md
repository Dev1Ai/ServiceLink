...
1.  **Fix Current Issues:** Resolve all existing linting errors (especially in `apps/marketing`) and test failures (by running `pnpm db:generate` to sync Prisma client).
2.  **Enforce CI:** Configure GitHub branch protection rules to require all CI status checks (lint, test, build) to pass before merging to the main branch.
3.  **Strict Database Workflow:** Always follow this sequence for schema changes: `prisma format` -> `pnpm db:push` (or `db:migrate`) -> `pnpm db:generate`.
4.  **Proactive Dependency Management:** Regularly check for and strategically update dependencies using `pnpm outdated` and `pnpm up --latest -i`, running tests after each update.

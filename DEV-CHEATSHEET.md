...
🔄 Migration workflow 1. Edit schema: nano apps/api/src/prisma/schema.prisma 2. Create/apply migration: pnpm db:migrate (change init in script or run full command manually with a descriptive name) 3. Regenerate client: pnpm prisma:gen 4. Seed data (if needed): pnpm db:seed
🧹 Resetting database
• Drops database
• Recreates schema

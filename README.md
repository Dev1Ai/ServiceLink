# ServiceLink — Real‑Time Local Services Marketplace (M4 Fulfillment)

ServiceLink is a "hire a pro like a ride‑share" marketplace built as a
Turborepo monorepo. The project has progressed through M0-M4 milestones,
implementing core job matching, scheduling automation, and payment capture
placeholders. This README reflects the current **M4 (Fulfillment & Automation)**
state.
...
**Latest Release Tag (proposal):** `v0.4.0-m4-fulfillment`

## Monorepo at a Glance

- **apps/api** – NestJS REST + WebSocket API with Prisma/Postgres. Includes
  job matching, scheduling automation with BullMQ reminders, and payment capture placeholders.
- **apps/web** – Next.js App Router admin/operations UI. Runs with a strict CSP.
- **apps/mobile-uber-polished** – Expo (SDK 54) mobile app client (early stage).
- **apps/mobile** – Original playground app kept for reference.
- **apps/marketing** – Static marketing site (Next.js export).
- **docs/** – Architecture notes and M4 milestone documentation (Fulfillment, Tickets, Runbooks).
- **infra/** – Docker Compose profiles and deployment manifests.
- **prisma/** – Database schema with migrations and seed script.
- **packages/schemas** – Shared Zod DTOs for API/web/mobile alignment.

See [Repository Guidelines](AGENTS.md) for contributor workflows, coding
standards, and release expectations.

## Milestone Progress (M0 → M4)

- ✅ **M0 Bootstrap** – Turborepo scaffold, Docker Compose (Postgres + Redis), health checks
- ✅ **M1 API Foundation** – JWT auth, jobs/quotes CRUD, Prisma/Postgres schema
- ✅ **M2 Realtime Presence** – Socket.IO namespace, presence tracking, chat messages
- ✅ **M3 Matching & Rate Limits** – Provider discovery (geo search), quote lifecycle,
  realtime job chat, static export-safe web routes
- ✅ **M4 Fulfillment** – Assignment scheduling with optimistic locking, BullMQ reminder
  worker, payment capture placeholders, payout status tracking
  (docs: `M4-Fulfillment.md`, `M4-Tickets.md`, `M4-Reminder-Worker-Runbook.md`)

**Next Milestone:** M5 — Payments & Reviews (Stripe PaymentIntents, Connect payouts, refunds, ratings/reviews)

See `PLAYBOOK.md` for full roadmap through M7.
...
| M4 Fulfillment Spec        | `docs/M4-Fulfillment.md`             |
| M4 Ticket Backlog          | `docs/M4-Tickets.md`                 |
| M4 Sprint Plan             | `docs/M4-Sprint-Plan.md`             |
| M4 Grooming Checklist      | `docs/M4-Grooming.md`                |
| M4 Coordination Checklist  | `docs/M4-Coordination.md`            |
| M4 Reminder Worker Runbook | `docs/M4-Reminder-Worker-Runbook.md` |
